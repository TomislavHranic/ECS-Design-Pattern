...
export default class ExampleScene2 extends Scene {
  ...
  init() {
    ...
    let block = this.entityManager.addBlock(3*16, 100);
    this.mapEntitiesToSystems(block, ['sRender', 'sSpritesheetAnimation', 'sCollisionDetection']);
    block = this.entityManager.addBlock(4*16, 120);
    this.mapEntitiesToSystems(block, ['sRender', 'sSpritesheetAnimation', 'sCollisionDetection']);
    block = this.entityManager.addBlock(5*16, 140);
    this.mapEntitiesToSystems(block, ['sRender', 'sSpritesheetAnimation', 'sCollisionDetection']);
    block = this.entityManager.addBlock(6*16, 160);
    this.mapEntitiesToSystems(block, ['sRender', 'sSpritesheetAnimation', 'sCollisionDetection']);
    block = this.entityManager.addBlock(7*16, 180);
    this.mapEntitiesToSystems(block, ['sRender', 'sSpritesheetAnimation', 'sCollisionDetection']);
